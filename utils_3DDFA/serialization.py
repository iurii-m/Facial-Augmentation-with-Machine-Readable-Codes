    ...
    for i, ver in enumerate(ver_lst):
        ...
        with open(wfp_new, 'w') as f:
            for i in range(n_vertex):
                ...
                subys = 150
                multys = 100
                x, y, z = (x-subys)/multys, (y-subys)/multys, (z-subys)/multys #multiply values 
                f.write(
...
def ser_to_obj_multiple(img, ver_lst, tri, height, wfp):
    ...
    with open(wfp, 'w') as f:
        for i in range(n_obj):
            ...
            for j in range(n_vertex):
                x, y, z = ver[:, j]
                x_max, y_max, z_max =  max(abs(x),x_max),max(abs(y),y_max),max(abs(z),z_max)
                x_center, y_center, z_center = x_center+x, y_center+y, z_center+z
            x_center, y_center, z_center = x_center/n_vertex, y_center/n_vertex, z_center/n_vertex

            for j in range(n_vertex):
                x, y, z = ver[:, j]
                x, y, z = (x-x_center)/(x_max-x_center), (y-y_center)/(y_max-y_center), 0.7*(z-z_center*0)/(z_max-z_center) #multiply values 
                f.write(
                    #f'v {x:.2f} {height - y:.2f} {z:.2f} {colors[j, 2]:.2f} {colors[j, 1]:.2f} {colors[j, 0]:.2f}\n')
                    f'v {x:.2f} {- y:.2f} {z:.2f} {colors[j, 2]:.2f} {colors[j, 1]:.2f} {colors[j, 0]:.2f}\n')
